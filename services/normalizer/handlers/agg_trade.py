...
def handle_agg_trade(event: RawEvent) -> HandlerResult:
    d = event.payload
    symbol = normalize_symbol(d.get("s", "")) or symbol_from_stream(event.source_stream)
    if not symbol:
        raise NormalizeError(f"agg_trade: cannot determine symbol from {event.source_stream!r}")
    ...
    normalized = NormalizedEvent(
        event_type=EventType.AGG_TRADE,
        venue=Venue.BINANCE,
        ...
        data={
            "agg_trade_id": d.get("a"),
            "price": price,
            "qty": qty,
            "first_trade_id": d.get("f"),
            "last_trade_id": d.get("l"),
